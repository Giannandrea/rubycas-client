...
= RubyCAS-Client 
This is a fork of the official RubyCAS-Client in order to run it on Rails4.
Below all informations about the autohor.
...
The current version of RubyCAS-Client should work with Rails 4.1.1 and up. For compatibility with
older Rails try using an older version of the client on http://github.com/rubycas/rubycas-client.

if you're using Rails, it's easier to install the CAS client as a gem with bundler. To do this add gem in your Gemfile

<i>gem 'rubycas-client', :git => 'https://github.com/Giannandrea/rubycas-client.git'</i>

For a first test you can install gem using standard gem
gem install rubycas-client -s https://github.com/Giannandrea/rubycas-client.git
...
https://github.com/Giannandrea/rubycas-client/tree/master/examples/rails4/
...
After installing RubyCAS-Client as a gem (see above), add the following to your app's <tt>config/initializers/cas_client.rb</tt>:
  
<i>  CASClient::Frameworks::Rails::Filter.configure(
    :cas_base_url => "https://cas.example.foo/"
  )</i>
  
(Change the <tt>:cas_base_url</tt> value to your CAS server's base URL; also note that many CAS servers like RubyCas-server are configured
